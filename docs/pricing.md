...
const pricingData = {
    ...
    "prices": [
        ...
        {"id": "claude-sonnet-5", "vendor": "anthropic", "name": "Claude Sonnet 5", "input": 3, "output": 15},
        ...
        {"id": "gpt-5.4-mini", "vendor": "openai", "name": "GPT-5.4 Mini", "input": 0.75, "output": 4.5},
        ...
        {"id": "gemini-3-pro-image", "vendor": "google", "name": "Gemini 3 Pro Image Preview", "input": 2.0, "output": 134.0},
        {"id": "gemini-3.1-flash-image", "vendor": "google", "name": "Gemini 3.1 Flash Image Preview*", "input": 0.5, "output": 67.2},
        {"id": "gemini-3.1-flash-lite-image", "vendor": "google", "name": "Gemini 3.1 Flash-Lite Image Preview*", "input": 0.25, "output": 33.6},
...
// Active vendor filter set by clicking legend vendor items (null = show all)
...
function getFilteredData() {
    const rawSearch = document.getElementById('pricingSearch').value.toLowerCase();
    const searchTerm = rawSearch.trim();
    const searchNormalized = searchTerm.replace(/[-_]/g, ' ');
...
function toggleVendorFilter(vendor) {
    activeVendorFilter = activeVendorFilter === vendor ? null : vendor;
    filterTable();
    buildLegend();
}
...
function buildLegend() {
    ...
    const vendorItems = [
        {color: '#8B5CF6', label: 'Google',    vendor: 'google'},
        {color: '#FFB800', label: 'Amazon',    vendor: 'amazon'},
        {color: '#1A1A1A', label: 'OpenAI',    vendor: 'openai'},
        {color: '#E8511A', label: 'Anthropic', vendor: 'anthropic'}
    ];
    const typeItems = [
    ...
    vendorItems.forEach(v => {
        const isActive = activeVendorFilter === v.vendor;
        const style = `cursor:pointer;padding:2px 8px;border-radius:4px;user-select:none;color:${v.color};${isActive ? 'background:#e0e0e0;font-weight:bold;' : ''}`;
        html += `<span style="${style}" onclick="toggleVendorFilter('${v.vendor}')" title="Click to filter">${v.label}</span>`;
    });
    if (activeVendorFilter) {
        html += `<span style="color:#666;margin-left:8px;font-style:italic;cursor:pointer;" onclick="toggleVendorFilter(activeVendorFilter)">✕ clear filter</span>`;
    }
    html += '</div>';

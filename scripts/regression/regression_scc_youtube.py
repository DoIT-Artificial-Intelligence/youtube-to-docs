"""
Regression test for --suggest-corrected-captions with a YouTube SRT source.

Fetches the YouTube transcript/SRT for the test video, then asks an LLM to
suggest WCAG 2.1 Level AA compliant corrections (punctuation, capitalisation).
No speaker extraction is run, so no [Name] labels are expected.
...
def main() -> None:
    print("=== Regression: --suggest-corrected-captions (YouTube source) ===\n")
    ...
    out = os.path.join(ARTIFACTS_DIR, "scc-youtube.csv")

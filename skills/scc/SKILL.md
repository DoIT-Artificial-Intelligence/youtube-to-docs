...
- Determine the `suggest_corrected_captions` value:
  ...
  - If "youtube" is mentioned as the source, append `-youtube` (e.g. `gemini-3.1-flash-lite-youtube`).
  - If an STT model is mentioned as the source (e.g. "gcp-chirp3"), append it (e.g. `gemini-3.1-flash-lite-gcp-chirp3`).
  ...
  - If no model is specified, default to `gemini-3.1-flash-lite`.
